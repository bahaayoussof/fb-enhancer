...
export function extractDescription(filePath) {
  ...
  if (ext === '.php') {
    if (basename.endsWith('.blade.php')) {
      ...
      const sections = (content.match(/@section\(\s*['"](\w+)['"]/g) || [])
        .map((s) => s.match(/['"](\w+)['"]/)?.[1])
        .filter(Boolean);
      const parts = [];
    ...
    if (parent === 'Model' || parent === 'Authenticatable') {
      ...
      const rels = (
        content.match(/\$this->(hasMany|hasOne|belongsTo|belongsToMany|morphMany|morphTo)\(/g) || []
      ).length;
    ...
    if (className && pubMethods.length > 0) {
      ...
      return cap(
        pubMethods.length > 4
          ? `${className}: ${display} + ${pubMethods.length - 4} more`
          : `${className}: ${display}`
      );
    }
  }
  ...
  if (
    ...
    if (basename === 'route.ts' || basename === 'route.js') {
      const methods = [
        ...new Set(
          (
            content.match(/export\s+(?:async\s+)?function\s+(GET|POST|PUT|PATCH|DELETE)/g) || []
          ).map((m) => m.match(/(GET|POST|PUT|PATCH|DELETE)/)?.[1])
        ),
      ].filter(Boolean);
    ...
    if (routeHits && routeHits.length > 0) {
      const methods = [
        ...new Set(
          routeHits.map((r) => r.match(/\.(get|post|put|patch|delete)/)?.[1]?.toUpperCase())
        ),
      ];
    ...
    if (content.includes('z.object') || content.includes('z.string')) {
      const schemas = (content.match(/(?:export\s+)?(?:const|let)\s+(\w+)\s*=\s*z\./g) || [])
        .map((s) => s.match(/(?:const|let)\s+(\w+)/)?.[1])
        .filter(Boolean);
      if (schemas.length)
        return cap(
          `Zod schemas: ${schemas.slice(0, 4).join(', ')}${schemas.length > 4 ? ` + ${schemas.length - 4} more` : ''}`
        );
    }
    // Exports summary
    const exports = (
      content.match(
        /export\s+(?:async\s+)?(?:function|class|const|interface|type|enum)\s+(\w+)/g
      ) || []
    )
      .map((e) => e.match(/(\w+)$/)?.[1])
      .filter(Boolean);
  ...
  if (ext === '.py') {
    ...
    if (content.includes('@shared_task') || content.includes('@app.task')) {
      const tasks = (content.match(/def\s+(\w+)/g) || [])
    ...
    const funcs = (content.match(/def\s+(\w+)/g) || [])
      .map((f) => f.match(/def\s+(\w+)/)?.[1])
      .filter((n) => n && !n.startsWith('_'));
    if (pyClass && funcs.length > 0)
      return cap(
        funcs.length > 4
          ? `${pyClass[1]}: ${funcs.slice(0, 4).join(', ')} + ${funcs.length - 4} more`
          : `${pyClass[1]}: ${funcs.join(', ')}`
      );
  ...
  if (ext === '.rs') {
    ...
    if (structM) {
      const methods = (content.match(/pub\s+(?:async\s+)?fn\s+(\w+)/g) || [])
        .map((m) => m.match(/fn\s+(\w+)/)?.[1])
        .filter(Boolean);
      return cap(
        methods.length
          ? `${structM[1]}: ${methods.slice(0, 4).join(', ')}`
          : `Struct: ${structM[1]}`
      );
    }
  ...
  if (ext === '.java') {
    ...
    const annotations = (
      content.match(
        /@(RestController|Controller|Service|Repository|Component|Entity|Configuration)/g
      ) || []
    ).map((a) => a.slice(1));
    ...
    const methods = (
      content.match(/public\s+(?:static\s+)?(?:\w+(?:<[\w,\s]+>)?)\s+(\w+)\s*\(/g) || []
    )
      .map((m) => m.match(/(\w+)\s*\(/)?.[1])
  ...
  if (ext === '.cs') {
    ...
    if (
      ...
      const actions = (content.match(/\[Http(Get|Post|Put|Patch|Delete)\]/g) || [])
        .map((a) => a.match(/Http(\w+)/)?.[1])
        .filter(Boolean);
      return cap(
        actions.length
          ? `API Controller: ${className} (${[...new Set(actions)].join(', ')})`
          : `Controller: ${className}`
      );
    }
    if (parent === 'DbContext' || content.includes('DbSet<')) {
      const sets = (content.match(/DbSet<(\w+)>/g) || [])
        .map((s) => s.match(/<(\w+)>/)?.[1])
        .filter(Boolean);
  ...
  if (ext === '.rb') {
    ...
    if (parent?.includes('Controller')) {
      const actions = (
        content.match(/def\s+(index|show|new|create|edit|update|destroy|\w+)/g) || []
      )
        .map((m) => m.match(/def\s+(\w+)/)?.[1])
        .filter((n) => n && !n.startsWith('_'));
  ...
  if (ext === '.dart') {
    if (content.includes('StatefulWidget') || content.includes('StatelessWidget')) {
      ...
      return name
        ? `${content.includes('StatefulWidget') ? 'Stateful' : 'Stateless'} widget: ${name[1]}`
        : 'Flutter widget';
    }
  ...
  if (ext === '.sql') {
    const creates = (content.match(/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?[`"']?(\w+)/gi) || [])
      .map((m) =>
        m.match(/(?:TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?)([`"']?\w+)/i)?.[1]?.replace(/[`"']/g, '')
      )
      .filter(Boolean);
  ...
  // ─── Proto / GraphQL ─────────────────────────────────────
  if (ext === '.proto') {
    const msgs = (content.match(/message\s+(\w+)/g) || [])
      .map((m) => m.match(/message\s+(\w+)/)?.[1])
      .filter(Boolean);
    const services = (content.match(/service\s+(\w+)/g) || [])
      .map((m) => m.match(/service\s+(\w+)/)?.[1])
      .filter(Boolean);
    const parts = [];
  ...
  if (ext === '.yaml' || ext === '.yml') {
    ...
    if (
      ...
      (basename.includes('docker') || basename.includes('compose'))
    ) {
  ...
  if (ext === '.ex' || ext === '.exs') {
    ...
    const fns = (content.match(/def\s+(\w+)/g) || [])
      .map((m) => m.match(/def\s+(\w+)/)?.[1])
      .filter(Boolean);
  ...
  if (declM) {
    const name = declM[1];
    const methods = (
      content.match(/(?:public\s+)?(?:async\s+)?(?:function\s+|(?:get|set)\s+)(\w+)\s*\(/g) || []
    )
      .map((m) => m.match(/(\w+)\s*\(/)?.[1])
