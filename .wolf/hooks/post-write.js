...
async function main() {
  ...
  try {
    const action =
      toolName === 'Write' ? 'Created' : toolName === 'MultiEdit' ? 'Multi-edited' : 'Edited';
    ...
    appendMarkdown(
      memoryPath,
      `| ${timeShort()} | ${action} ${relFile} | ${outcome} | ~${writeTokens} |\n`
    );
  } catch {}
...
function summarizeEdit(oldStr, newStr, filename) {
  ...
  const methodMatch = newStr.match(
...
function detectFixPattern(oldStr, newStr, ext) {
  ...
  const oldImports = new Set(
    (oldStr.match(/(?:import|require)\s*\(?['"]([^'"]+)['"]\)?/g) || []).map((m) => m)
  );
  ...
  if (ext === '.ts' || ext === '.tsx') {
    if (
      (newStr.includes(' as ') && !oldStr.includes(' as ')) ||
  ...
  if (ext === '.css' || ext === '.scss' || ext === '.vue' || ext === '.tsx' || ext === '.jsx') {
    ...
    const changedProps = [...newProps.entries()].filter(
      ([k, v]) => oldProps.get(k) !== v && oldProps.has(k)
    );
  ...
  if (diffRatio > 0.3 && oldLines.length >= 3 && newLines.length >= 3) {
    ...
    const removedLines = oldLines.filter(
      (l) => l.trim() && !newLines.some((nl) => nl.trim() === l.trim())
    );
